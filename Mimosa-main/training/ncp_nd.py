...
class MJnet(nn.Module):
    ...
    def forward(self, NCP_m, NCP_mi, ND_m, ND_mi):
        ...
        m_input = torch.cat(( NCP_m, ND_m.unsqueeze(-1)), dim=-1)  # (batch_size, seq_len, 7)
        mi_input = torch.cat(( NCP_mi, ND_mi.unsqueeze(-1)), dim=-1)  # (batch_size, seq_len, 7)
...
def perform_train(filepath):
    ...
    model = MJnet(input_size=4, hidden_size=128, num_layers=2, num_heads=8, dropout=0.4, output_size=1).to(device)
...
def kmers_predict(kmers,mirna,model):
    ...
    if len(kmers) == 0:
        ...
        model.eval()
        pros = model(fea_NCP_m, fea_NCP_mi, fea_ND_m, fea_ND_mi).detach().cpu().numpy().tolist()
...
def perform_test(pathfile, stepsize, model_type):
    ...
    model = MJnet(input_size=4, hidden_size=128, num_layers=2, num_heads=8, dropout=0.4, output_size=1)  # 创建模型实例
