...
class MJnet(nn.Module):
    ...
    def forward(self, C2_m, C2_mi, NCP_m, NCP_mi):

       
        m_input = torch.cat((C2_m, NCP_m), dim=-1)  # (batch_size, seq_len, 7)
        mi_input = torch.cat((C2_mi, NCP_mi), dim=-1)  # (batch_size, seq_len, 7)
...
def kmers_predict(kmers,mirna,model):
    ...
    if len(kmers) == 0:
        ...
        model.eval()
        pros = model(fea_C2_m, fea_C2_mi, fea_NCP_m, fea_NCP_mi).detach().cpu().numpy().tolist()
