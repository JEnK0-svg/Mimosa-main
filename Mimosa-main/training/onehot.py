...
class MJnet(nn.Module):
    ...
    def forward(self, Onehot_m, Onehot_mi):


        m_input = Onehot_m  # (batch_size, seq_len, 7)
        mi_input = Onehot_mi  # (batch_size, seq_len, 7)
...
def kmers_predict(kmers,mirna,model):
    ...
    if len(kmers) == 0:
        ...
        model.eval()
        pros = model(fea_Onehot_m, fea_Onehot_mi).detach().cpu().numpy().tolist()
