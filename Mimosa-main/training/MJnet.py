...
class MJnet(nn.Module):
    ...
    def forward(self, onehot_m, onehot_mi, pairing_m, pairing_mi):
        # 将pairing特征与one-hot编码拼接，形成5维输入 (4维one-hot + 1维pairing)
        m_input = torch.cat((onehot_m, pairing_m.unsqueeze(-1)), dim=-1)  # (batch_size, seq_len, 5)
        mi_input = torch.cat((onehot_mi, pairing_mi.unsqueeze(-1)), dim=-1)
