...
class MJnet(nn.Module):
    ...
    def forward(self, C2_m, C2_mi):
        ...
        # Cross attention mechanism
        cross_attend, _ = self.cross_attention(m_cnn_output.permute(1, 0, 2), mi_cnn_output.permute(1, 0, 2), mi_cnn_output.permute(1, 0, 2))
...
def perform_train(filepath):
    ...
    model = MJnet(input_size=2, hidden_size=128, num_layers=2, num_heads=8, dropout=0.4, output_size=1).to(device)
